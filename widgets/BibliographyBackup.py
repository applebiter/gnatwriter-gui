...
class Bibliography(QWidget):
    ...
    def __init__(self, story_id=None, bibliography_id=None, backend=None):
        ...
        sizePolicy.setHeightForWidth(self.lst_bibliographies.sizePolicy().hasHeightForWidth())
        self.lst_bibliographies.setSizePolicy(sizePolicy)
        ...
        # The delete button
        self.hbx_delete_bibliography = QHBoxLayout()
        self.horizontalSpacer = QSpacerItem(40, 20, QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Minimum)
        self.hbx_delete_bibliography.addItem(self.horizontalSpacer)
        self.btn_delete = QPushButton("Delete")
        self.btn_delete.clicked.connect(self.delete_bibliography)
        self.hbx_delete_bibliography.addWidget(self.btn_delete)
        self.vbx_outermost.addLayout(self.hbx_delete_bibliography)
        ...
        self.btn_save.clicked.connect(self.save_bibliography)
    ...
    def load_model(self):
        ...
        all_bibliographies = self.gnaw("bibliography").get_all_submissions()
        for bibliography in all_bibliographies:
            bibliography_item = QStandardItem(bibliography.submitted_to)
            # Chose 14 to represent the data index holding the identifier
            bibliography_item.setData(bibliography.id, 14)
            self.model.appendRow(bibliography_item)
        self.selection_model = self.lst_bibliographies.selectionModel()
        self.selection_model.selectionChanged.connect(self.item_selected)
    ...
    def setup_form(self):
        if self.current_bibliography_id:
            bibliography = self.gnaw("bibliography").get_bibliography_by_id(self.current_bibliography_id)
            if bibliography:
                self.ldt_submitted_to.setText(bibliography.submitted_to)
                if bibliography.date_sent:
                    self.date_sent = bibliography.date_sent
                    self.ddt_date_sent.setDate(QDate.fromString(str(bibliography.date_sent), 'yyyy-MM-dd'))
                ...
                if bibliography.date_reply_received:
                    self.date_reply_received = bibliography.date_reply_received
                    self.ddt_date_reply_received.setDate(QDate.fromString(str(bibliography.date_reply_received), 'yyyy-MM-dd'))
                ...
                if bibliography.date_published:
                    self.date_published = bibliography.date_published
                    self.ddt_date_published.setDate(QDate.fromString(str(bibliography.date_published), 'yyyy-MM-dd'))
                ...
                if bibliography.date_paid:
                    self.date_paid = bibliography.date_paid
                    self.ddt_date_paid.setDate(QDate.fromString(str(bibliography.date_paid), 'yyyy-MM-dd'))
                ...
                self.cbx_result.setCurrentIndex(self.cbx_result.findText(bibliography.result))
                self.ldt_amount.setText(str(bibliography.amount) if bibliography.amount else "")
    ...
    def save_bibliography(self):
        ...
        if self.current_bibliography_id:
            self.gnaw("bibliography").update_bibliography(
                submission_id=self.current_bibliography_id,
            ...
            bibliography = self.gnaw("bibliography").create_bibliography(
            ...
            if bibliography:
                self.current_bibliography_id = bibliography.id
        self.load_model()

    def delete_bibliography(self):
        if self.current_bibliography_id:
            self.gnaw("bibliography").delete_bibliography_by_id(self.current_bibliography_id)
            self.current_bibliography_id = None
            self.clear_form()
            self.load_model()
